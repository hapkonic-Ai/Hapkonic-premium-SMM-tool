...
const platforms = [
  { id: "instagram", name: "Instagram", icon: <Instagram className="w-5 h-5 text-pink-500" />, color: "border-pink-500/20 bg-pink-500/5", connected: false },
  { id: "facebook", name: "Facebook", icon: <Facebook className="w-5 h-5 text-blue-500" />, color: "border-blue-500/20 bg-blue-500/5", connected: true, username: "@hapkonic_official" },
  { id: "twitter", name: "Twitter", icon: <Twitter className="w-5 h-5 text-cyan-400" />, color: "border-cyan-400/20 bg-cyan-400/5", connected: false },
  { id: "linkedin", name: "LinkedIn", icon: <Linkedin className="w-5 h-5 text-blue-600" />, color: "border-blue-600/20 bg-blue-600/5", connected: false },
];
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {platforms.map((platform, i) => (
          ...
          >
            <GlassCard className={cn("p-6 border-white/5 h-full flex flex-col justify-between", platform.color)}>
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {platform.connected ? (
                      <p className="text-xs text-accent-green font-mono">{platform.username}</p>
                    ) : (
              ...
              <div className="space-y-4">
                {platform.connected ? (
                  <>
                    <div className="p-3 rounded-lg bg-black/20 border border-white/5">
                      ...
                      <div className="flex justify-between items-center text-[10px] uppercase font-bold tracking-widest text-text-muted">
                         ...
                         <span className="text-accent-cyan">Read/Write</span>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="secondary" size="sm" className="flex-1 text-[10px] h-9">
                        <ExternalLink className="w-3 h-3 mr-2" /> REFRESH
                      </Button>
                  ...
                  <Button 
                    className="w-full h-11 text-xs font-black uppercase tracking-widest bg-white/5 border-white/10 hover:bg-white/10"
